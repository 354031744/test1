package io.chengfeng.cms.admin.common.ueditor.upload;

import io.chengfeng.cms.admin.common.config.UploadConfig;
...
//			UploadResult result = baseFileService.upload(dataBuf, picName, "OM", null);
            ...
            //如果上传成功
            if (success) {
                uploadFile(dataBuf, path, picName);
                System.out.println("path:" + path + ",picName:" + picName);
                state = new BaseState(true);
                String url = UploadConfig.resourceUrl + path.substring(path.lastIndexOf("upload") + 7, path.length()) + "/" + picName;
